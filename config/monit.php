...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | 反向代理信任（X-Forwarded-* 头）
    |--------------------------------------------------------------------------
    | X-Forwarded-For/Proto 头仅当 REMOTE_ADDR 命中可信代理时生效，否则客户端
    | 可伪造该头污染统计访客 IP / 绕过 IP 限流。消费方：AppServiceProvider::boot()
    |（不能放 bootstrap/app.php 的 withMiddleware——该闭包在 kernel 构造期执行，
    |  早于 .env/config 加载，env() 只能拿到默认值）。
    |   private（默认）= 信任本机与私网段（nginx/容器同机部署的常见形态）
    |   逗号分隔 IP/CIDR = 信任指定代理（公网 LB/CDN 场景填其出口 IP 段）
    |   *             = 信任所有来源的头（仅纯内网部署使用，公网可伪造）
    |   none          = 不信任任何代理头（应用直接对外时最安全）
    | 注：不能用留空表示关闭——Laravel env() 会把空值当未设置而回落默认值
    */
    'trusted_proxies' => env('TRUSTED_PROXIES', 'private'),
