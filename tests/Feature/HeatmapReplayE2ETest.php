...
class HeatmapReplayE2ETest extends TestCase
{
    ...
    public function test_heatmap_check_auto_creates_heatmap_and_snapshot_pipeline_works(): void
    {
        ...
        $heatmapId = $check->json('heatmap_id');
        $this->assertNotNull($heatmapId);
        ...
        $this->assertSame(1, HeatmapSnapshotScroll::where('website_id', $this->website->website_id)->count());

        // 4. 用户端：热图详情页渲染 + 快照端点返回 rrweb 事件 + AJAX 返回坐标
        $this->actingAs($this->user)
            ->get('/stats/'.$this->website->website_id.'/heatmaps/'.$heatmapId)
            ->assertOk();

        $snapshot = $this->actingAs($this->user)
            ->get('/stats/'.$this->website->website_id.'/heatmaps/'.$heatmapId.'/snapshot?device=desktop');
        $snapshot->assertOk();
        $this->assertCount(2, $snapshot->json('events'));

        $ajax = $this->actingAs($this->user)
            ->get('/stats/'.$this->website->website_id.'/heatmaps-ajax/'.$heatmapId);
