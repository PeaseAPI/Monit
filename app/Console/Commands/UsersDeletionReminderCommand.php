...
class UsersDeletionReminderCommand extends Command
{
    ...
    {
        $autoDeleteDays = Typed::int(Setting::query()->where('key', 'users.auto_delete_inactive_users')->value('value') ?? 0);
        ...
        $reminderDays = Typed::int(Setting::query()->where('key', 'users.user_deletion_reminder')->value('value') ?? 7);
