...
\App\Support\Settings::set('offload.is_enabled', true);
...
if (! function_exists('monit_offload_asset')) {
    ...
    {
        if (! \App\Support\PluginManager::isActive('offload')) {
            return $path;
        }

        $cdn = rtrim((string) \App\Support\PluginManager::setting('offload', 'cdn_url', ''), '/');
...
\Illuminate\Support\Facades\Blade::directive('offloadAsset', function (string $expression) {
