@extends('layouts.app')
@section('title', __('stats.pageviews_advanced'))
@section('content')
<div class="py-8">
    <x-stats-header :website="$website" :title="__('stats.pageviews_advanced')" />
    ...
    <div class="mt-6 grid gap-4 sm:grid-cols-3">
        ...
        @include('components.stat-card', ['label' => __('stats.bounce_rate'), 'value' => ($overview['bounce_rate'] ?? 0) . '%', 'change' => $overview['bounce_rate_change'] ?? null])
    </div>
    ...
    <div class="mt-6 rounded-2xl border border-zinc-200 bg-white">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-zinc-50 text-left"><tr><th class="px-6 py-3 font-medium text-zinc-500">{{ __('stats.page_path') }}</th><th class="px-6 py-3 font-medium text-zinc-500">{{ __('stats.pageviews') }}</th><th class="px-6 py-3 font-medium text-zinc-500">{{ __('stats.visitors') }}</th><th class="px-6 py-3 font-medium text-zinc-500">{{ __('stats.bounce_rate') }}</th></tr></thead>
                <tbody class="divide-y divide-zinc-100">
                    @forelse($topPaths ?? [] as $item)
                    <tr><td class="px-6 py-3 font-medium text-zinc-900">{{ $item['path'] }}</td><td class="px-6 py-3 text-zinc-500">{{ $item['count'] }}</td><td class="px-6 py-3 text-zinc-500">{{ $item['visitors'] ?? '-' }}</td><td class="px-6 py-3 text-zinc-500">{{ ($item['bounce_rate'] ?? 0) }}%</td></tr>
