...
{{-- main_class：覆盖布局默认内容栏，hero 全宽贴边（下方内容自行限宽） --}}
@section('main_class', 'w-full')
@section('content')
@php
    /* 分类图标映射：仅用于分类标题（工具卡片已改用 x-seo-tool-icon 组件的逐工具独立图标） */
    $catIcons = [
                       ...
                       data-cat="{{ $category }}">
                        <div class="flex items-start gap-3">
                            <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-zinc-100 text-zinc-600 transition group-hover:bg-brand-50 group-hover:text-brand-600">
                                {{-- 每个工具按 slug 定位独立图标（Heroicons outline） --}}
                                <x-seo-tool-icon :slug="$slug" class="h-5 w-5"/>
                            </span>
                            <div class="min-w-0">
