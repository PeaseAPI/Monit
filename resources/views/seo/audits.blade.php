...
<div class="max-w-7xl">
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <a href="{{ route('seo.audits.compare') }}" class="rounded-lg border border-zinc-200 px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-50">{{ __('seo.compare_title') }}</a>
    ...
    {{-- 批量重新审计提交表单（各行 checkbox 通过 form 属性关联提交） --}}
    <form id="bulk-refresh-form" method="POST" action="{{ route('seo.audits.bulk-refresh') }}" class="hidden">
        @csrf
    </form>
    ...
    <div class="mt-6 overflow-hidden rounded-2xl border border-zinc-200 bg-white">
        <table class="w-full text-sm">
            ...
            <tr>
                <th class="px-6 py-3 w-10"><input type="checkbox" id="select-all-audits"></th>
            ...
            @forelse($audits as $audit)
                <tr>
                    <td class="px-6 py-3"><input type="checkbox" name="audit_ids[]" value="{{ $audit->seo_audit_id }}" form="bulk-refresh-form"></td>
            ...
            @empty
                <tr><td colspan="6" class="px-6 py-8 text-center text-zinc-500">{{ __('seo.no_audits') }}</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
    <div class="mt-3 flex flex-wrap items-center justify-between gap-3">
        {{ $audits->links() }}
        <button type="submit" form="bulk-refresh-form" class="rounded-lg border border-zinc-200 px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-50">{{ __('seo.bulk_refresh') }}</button>
    </div>
</div>

<script>
document.getElementById('select-all-audits')?.addEventListener('change', function() {
    document.querySelectorAll('input[name="audit_ids[]"]').forEach(function(cb) { cb.checked = this.checked; }.bind(this));
});
</script>
@endsection
