...
<div class="p-8">
    ...
    <div class="rounded-2xl border border-zinc-200 bg-white overflow-x-auto">
        ...
        <tbody class="divide-y divide-zinc-100">
            @forelse($clicks ?? [] as $c)<tr><td class="px-6 py-3">@if(!empty($c->host))<a href="{{ route('stats.outbound_click_paths', ['website' => $website->website_id, 'host' => $c->host]) }}" class="text-brand-600 hover:underline">{{ $c->url }}</a>@else{{ $c->url }}@endif</td><td class="px-6 py-3">{{ $c->count }}</td><td class="px-6 py-3 text-zinc-500">{{ $c->date }}</td></tr>
