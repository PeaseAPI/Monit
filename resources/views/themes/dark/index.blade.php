{{-- ============================================================
  Monit 深色落地页主题（周期 #19 #20：dark 版，基于 default 主题色彩翻转）
...
<body class="bg-zinc-950 font-sans text-zinc-50 antialiased">
    ...
    {{-- ===== 顶部导航 ===== --}}
    <header class="sticky top-0 z-40 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-lg">
        <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
            <x-brand-logo href="{{ route('index') }}" />

            <nav class="hidden items-center gap-8 md:flex">
                <a href="#features" class="text-sm font-medium text-zinc-300 transition hover:text-zinc-50">{{ __('landing.nav_features') }}</a>
                <a href="#showcase" class="text-sm font-medium text-zinc-300 transition hover:text-zinc-50">{{ __('landing.nav_showcase') }}</a>
                <a href="#pricing" class="text-sm font-medium text-zinc-300 transition hover:text-zinc-50">{{ __('landing.nav_pricing') }}</a>
                ...
                <a href="{{ route('seo.tools') }}" class="text-sm font-medium text-zinc-300 transition hover:text-zinc-50">{{ __('landing.nav_seo_tools') }}</a>
                @endif
                @if (filter_var(\App\Support\Settings::get('seo.audits_is_enabled', true), FILTER_VALIDATE_BOOLEAN))
                <a href="{{ route('seo.directory') }}" class="text-sm font-medium text-zinc-300 transition hover:text-zinc-50">{{ __('landing.nav_seo_directory') }}</a>
                @endif
                <a href="{{ route('blog') }}" class="text-sm font-medium text-zinc-300 transition hover:text-zinc-50">{{ __('landing.nav_blog') }}</a>
                <a href="{{ route('help') }}" class="text-sm font-medium text-zinc-300 transition hover:text-zinc-50">{{ __('landing.nav_help') }}</a>
            </nav>
            ...
            <div class="flex items-center gap-3">
                ...
                <details class="group relative">
                    <summary class="flex cursor-pointer list-none items-center gap-1.5 rounded-xl border border-zinc-800 px-3 py-2 text-sm text-zinc-300 transition hover:border-zinc-300 hover:text-zinc-50 [&::-webkit-details-marker]:hidden">
                        ...
                        <svg class="h-3.5 w-3.5 text-zinc-500 transition group-open:rotate-180" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m19 9-7 7-7-7"/></svg>
                    </summary>
                    <div class="absolute right-0 z-50 mt-2 w-44 overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950 py-1 shadow-lg shadow-black/40">
                        @foreach (config('monit.locales') as $code => $meta)
                        <a href="{{ route('locale.switch', $code) }}"
                            class="flex items-center justify-between px-4 py-2 text-sm {{ $code === app()->getLocale() ? 'bg-brand-50 font-medium text-brand-700' : 'text-zinc-300 hover:bg-zinc-900 hover:text-zinc-50' }}">
                ...
                <a href="{{ route('login') }}" class="hidden text-sm font-medium text-zinc-300 transition hover:text-zinc-50 sm:block">{{ __('landing.nav_login') }}</a>
    ...
    <section class="relative overflow-hidden">
        ...
        <div class="mx-auto max-w-7xl px-6 pb-20 pt-24 text-center md:pt-32">
            ...
            <h1 class="mx-auto max-w-4xl text-4xl leading-tight font-bold tracking-tight text-zinc-50 md:text-6xl md:leading-[1.1]">
            ...
            <div class="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
                ...
                <a href="#showcase" class="w-full rounded-2xl border border-zinc-800 bg-zinc-950 px-8 py-4 text-base font-semibold text-zinc-300 transition hover:border-zinc-300 hover:bg-zinc-900 sm:w-auto">
                    {{ __('landing.cta_secondary') }}
                </a>
            </div>
            <p class="mt-4 text-sm text-zinc-500">{{ __('landing.no_card_required') }}</p>

            {{-- 免费 SEO 分析（对标 monit.cn 首页获客组件：POST /seo/analyze，审计开关控制） --}}
            @if (filter_var(\App\Support\Settings::get('seo.audits_is_enabled', true), FILTER_VALIDATE_BOOLEAN))
            <div class="mx-auto mt-12 max-w-xl">
                <form method="POST" action="{{ route('seo.analyze') }}" class="flex flex-col gap-2 rounded-2xl border border-zinc-800 bg-zinc-950 p-2 shadow-lg shadow-black/40 sm:flex-row" data-seo-form>
                    @csrf
                    <input type="text" name="url" required placeholder="{{ __('landing.seo_analyze_placeholder') }}" value="{{ old('url') }}"
                           class="flex-1 rounded-xl border-0 px-4 py-3 text-sm text-zinc-50 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-brand-500">
                ...
                <p class="mt-2 text-xs text-zinc-500">{{ __('landing.seo_analyze_desc') }}</p>
            ...
            <dl class="mx-auto mt-10 grid max-w-2xl grid-cols-2 gap-4 sm:grid-cols-4">
                <div class="rounded-2xl border border-zinc-800 bg-zinc-950/70 px-4 py-3 backdrop-blur">
                    <dt class="text-xs text-zinc-500">{{ __('landing.stats_websites') }}</dt>
                    <dd class="mt-0.5 text-xl font-bold tabular-nums text-zinc-50">{{ number_format($stats['websites']) }}+</dd>
                </div>
                <div class="rounded-2xl border border-zinc-800 bg-zinc-950/70 px-4 py-3 backdrop-blur">
                    <dt class="text-xs text-zinc-500">{{ __('landing.stats_pageviews') }}</dt>
                    <dd class="mt-0.5 text-xl font-bold tabular-nums text-zinc-50">{{ $stats['pageviews'] >= 1000 ? round($stats['pageviews'] / 1000).'K+' : number_format($stats['pageviews']) }}</dd>
                </div>
                <div class="rounded-2xl border border-zinc-800 bg-zinc-950/70 px-4 py-3 backdrop-blur">
                    <dt class="text-xs text-zinc-500">{{ __('landing.stats_retention') }}</dt>
                    <dd class="mt-0.5 text-xl font-bold tabular-nums text-zinc-50">365{{ __('landing.stats_days') }}</dd>
                </div>
                <div class="rounded-2xl border border-zinc-800 bg-zinc-950/70 px-4 py-3 backdrop-blur">
                    <dt class="text-xs text-zinc-500">{{ __('landing.stats_uptime') }}</dt>
                    <dd class="mt-0.5 text-xl font-bold tabular-nums text-zinc-50">99.9%</dd>
                </div>
            </dl>
            @endif
            ...
            <div class="relative mx-auto mt-16 max-w-5xl pb-10 [perspective:1600px]">
                ...
                <div class="rounded-2xl border border-zinc-800 bg-zinc-950 shadow-2xl shadow-zinc-900/10 [transform:rotateX(5deg)] transition-transform duration-700 hover:[transform:rotateX(0deg)]">
                    <div class="flex items-center gap-2 border-b border-zinc-800 px-4 py-3">
                        <span class="h-3 w-3 rounded-full bg-red-400"></span>
                        <span class="h-3 w-3 rounded-full bg-yellow-400"></span>
                        <span class="h-3 w-3 rounded-full bg-green-400"></span>
                        <span class="ml-4 rounded-md bg-zinc-900 px-3 py-1 text-xs text-zinc-500">{{ \App\Support\Brand::name() }} · {{ __('landing.mock_dashboard') }}</span>
                    </div>
                    <div class="grid gap-4 p-6 text-left md:grid-cols-4">
                        ...
                        <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
                            <p class="text-2xl font-bold text-zinc-50">{{ $stat['value'] }}</p>
                        ...
                        @endforeach
                        <div class="relative col-span-full h-44 overflow-hidden rounded-xl border border-zinc-800 bg-gradient-to-b from-brand-50/50 to-white p-4">
    ...
    <section id="features" class="border-t border-zinc-800 bg-zinc-900/50 py-24">
        <div class="mx-auto max-w-7xl px-6">
            <div class="mx-auto max-w-2xl text-center">
                ...
                <h2 class="mt-3 text-3xl font-bold tracking-tight text-zinc-50 md:text-4xl">{{ __('landing.features_title') }}</h2>
            ...
            <div class="reveal mt-16 grid gap-6 md:grid-cols-3">
                ...
                <div class="group rounded-2xl border border-zinc-800 bg-zinc-950 p-8 transition hover:-translate-y-1 hover:border-brand-200 hover:shadow-xl hover:shadow-brand-600/5">
                    ...
                    <h3 class="mt-5 text-lg font-semibold text-zinc-50">{{ $feature['title'] }}</h3>
    ...
    <section id="showcase" class="relative overflow-hidden bg-zinc-950 py-24">
        ...
        <div class="relative mx-auto max-w-7xl px-6">
            <div class="mx-auto max-w-2xl text-center">
                ...
                <p class="mt-4 text-lg text-zinc-500">{{ __('landing.why_subtitle') }}</p>
            </div>
            ...
            <div class="reveal mt-20 grid gap-6 md:grid-cols-3">
                ...
                <div class="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-8">
                    ...
                    <p class="mt-2 text-sm leading-relaxed text-zinc-500">{{ $step['desc'] }}</p>
                </div>
                @endforeach
            </div>
        </div>
    ...
    <section id="pricing" class="py-24">
        <div class="mx-auto max-w-7xl px-6">
            <div class="mx-auto max-w-2xl text-center">
                <h2 class="text-3xl font-bold tracking-tight text-zinc-50 md:text-4xl">{{ __('landing.pricing_title') }}</h2>
                ...
                <form method="GET" action="{{ route('index') }}" class="mt-6 inline-flex items-center gap-2" id="landing-currency-form">
                    ...
                    <select id="landing-currency" name="currency" onchange="this.form.submit()"
                        class="rounded-lg border border-zinc-300 bg-zinc-950 px-3 py-1.5 text-sm text-zinc-300">
                ...
                <div id="billing-toggle" class="mt-8 inline-flex items-center gap-1 rounded-full border border-zinc-800 bg-zinc-900 p-1">
                    <button type="button" data-freq="monthly" aria-pressed="true"
                        class="rounded-full bg-zinc-950 px-5 py-1.5 text-sm font-semibold text-zinc-50 shadow-sm">{{ __('landing.billing_monthly') }}</button>
                    <button type="button" data-freq="annual" aria-pressed="false"
                        class="rounded-full px-5 py-1.5 text-sm font-medium text-zinc-500 transition hover:text-zinc-50">
            ...
            <div class="reveal mt-16 grid gap-6 md:grid-cols-3">
                ...
                <div class="{{ $featured ? 'relative rounded-2xl border-2 border-brand-600 bg-zinc-950 p-8 shadow-xl shadow-brand-600/10 md:-translate-y-3' : 'rounded-2xl border border-zinc-800 bg-zinc-950 p-8' }}">
                    @if ($featured)
                    ...
                    @endif
                    <h3 class="text-lg font-semibold text-zinc-50">{{ $plan->name }}</h3>
                    <p class="mt-3">
                        @if ($plan->landing_price !== null && (float) $plan->landing_price > 0)
                            <span data-price="monthly" class="text-4xl font-bold text-zinc-50">{{ \App\Support\Currency::format((float) $plan->landing_price, $planCode) }}</span>
                            <span data-price="monthly" class="text-sm font-normal text-zinc-500">/{{ __('landing.per_month') }}</span>
                            @if ($annual !== null && (float) $annual > 0)
                            <span data-price="annual" class="hidden text-4xl font-bold text-zinc-50">{{ \App\Support\Currency::format((float) $annual, $planCode) }}</span>
                            <span data-price="annual" class="hidden text-sm font-normal text-zinc-500">/{{ __('landing.per_year') }}</span>
                            @if ($savePct > 0)
                            ...
                            @endif
                        @else
                            <span class="text-4xl font-bold text-zinc-50">{{ __('landing.free') }}</span>
                        @endif
                    </p>
                    ...
                    @if (!empty($s))
                    <ul class="mt-5 space-y-2.5 border-t border-zinc-800 pt-5 text-sm text-zinc-300">
                    ...
                    <a href="{{ route('register') }}" class="{{ $featured ? 'mt-6 block rounded-xl bg-brand-600 px-6 py-3 text-center text-sm font-semibold text-white transition hover:bg-brand-700' : 'mt-6 block rounded-xl border border-zinc-300 px-6 py-3 text-center text-sm font-semibold text-zinc-300 transition hover:bg-zinc-900' }}">
                    ...
                    buttons.forEach(function (btn) {
                        btn.addEventListener('click', function () {
                            ...
                            buttons.forEach(function (b) {
                                ...
                                b.classList.toggle('bg-zinc-950', on);
                                b.classList.toggle('shadow-sm', on);
                                b.classList.toggle('font-semibold', on);
                                b.classList.toggle('text-zinc-50', on);
    ...
    @if ($showTestimonials)
    <section class="relative overflow-hidden border-t border-zinc-800 bg-zinc-900/50 py-20 md:py-24">
        ...
        <div class="relative mx-auto max-w-7xl px-6">
            <div class="mx-auto max-w-2xl text-center">
                ...
                <h2 class="mt-3 text-3xl font-bold tracking-tight text-zinc-50 md:text-4xl">{{ __('landing.testimonials_title') }}</h2>
            ...
            <div class="reveal mt-14 grid gap-4 sm:grid-cols-3">
                ...
                <div class="flex items-center gap-3 rounded-2xl border border-zinc-800/70 bg-zinc-950/70 px-5 py-4 backdrop-blur transition hover:border-brand-200">
                    ...
                    <p class="text-sm font-medium text-zinc-300">{{ __($badge['key']) }}</p>
                </div>
                @endforeach
            </div>
            ...
            <div class="reveal mt-10 grid gap-6 md:grid-cols-3">
                ...
                <figure class="flex h-full flex-col justify-between rounded-2xl border border-zinc-800/70 bg-zinc-950/70 p-7 backdrop-blur transition hover:border-brand-200">
                    <blockquote class="text-sm leading-relaxed text-zinc-300">“{{ __($testimonial['quote']) }}”</blockquote>
                    <figcaption class="mt-5 flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-zinc-50">{{ __($testimonial['author']) }}</p>
                            <p class="text-xs text-zinc-500">{{ __($testimonial['role']) }}</p>
                        </div>
                    </figcaption>
                </figure>
                @endforeach
    ...
    @if ($showFaq)
    <section class="border-t border-zinc-800 bg-zinc-950 py-20 md:py-24">
        <div class="mx-auto grid max-w-6xl gap-12 px-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
            <div class="lg:sticky lg:top-24 lg:self-start">
                ...
                <h2 class="mt-3 text-3xl font-bold tracking-tight text-zinc-50">{{ __('landing.faq_title') }}</h2>
                <p class="mt-4 text-base leading-relaxed text-zinc-500">{{ __('landing.subtitle') }}</p>
                <a href="{{ route('contact') }}" class="mt-6 inline-flex items-center gap-2 rounded-xl border border-zinc-300 px-5 py-2.5 text-sm font-semibold text-zinc-300 transition hover:border-brand-500 hover:text-brand-600">
                ...
                <p class="mt-3 text-xs text-zinc-500">{{ __('landing.faq_still_questions') }}</p>
            </div>
            <div class="space-y-3.5">
                ...
                <details class="group rounded-2xl border border-zinc-800 bg-zinc-900/60 transition open:border-brand-200 open:bg-zinc-950 open:shadow-sm">
                    <summary class="flex cursor-pointer list-none items-center gap-4 p-5 text-sm font-semibold text-zinc-50 [&::-webkit-details-marker]:hidden">
                    ...
                    <p class="border-t border-zinc-800 px-5 py-4 pl-16 text-sm leading-relaxed text-zinc-500">{{ $a }}</p>
                </details>
                @endforeach
            </div>
        </div>
    ...
    @if (in_array($showLatestPosts, [true, 1, '1', 'true', 'on'], true))
        ...
        <section class="border-t border-zinc-800 bg-zinc-950 py-16 md:py-20">
            <div class="mx-auto max-w-7xl px-6">
                <div class="flex items-end justify-between">
                    <div>
                        <p class="text-sm font-semibold tracking-widest text-brand-600 uppercase">{{ __('landing.latest_posts_eyebrow') }}</p>
                        <h2 class="mt-2 text-3xl font-bold tracking-tight text-zinc-50">{{ __('landing.latest_posts_title') }}</h2>
                    </div>
                ...
                <div class="mt-10 grid gap-6 md:grid-cols-3">
                    @foreach ($latestPosts as $post)
                        <a href="{{ route('blog.post', $post->url) }}" class="group rounded-2xl border border-zinc-800 bg-zinc-950 p-6 transition hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-lg hover:shadow-brand-600/5">
                            <time class="text-xs font-medium text-zinc-500" datetime="{{ $post->datetime?->toIso8601String() }}">{{ $post->datetime?->format('Y-m-d') }}</time>
                            <h3 class="mt-2 line-clamp-2 font-semibold text-zinc-50 transition group-hover:text-brand-600">{{ $post->title }}</h3>
    ...
    <section class="py-20 md:py-24">
        <div class="mx-auto max-w-7xl px-6">
            <div class="relative overflow-hidden rounded-3xl bg-zinc-950 px-8 py-16 text-center md:py-20">
                ...
                <p class="relative mx-auto mt-4 max-w-xl text-lg text-zinc-500">{{ __('landing.cta_subtitle') }}</p>
                <div class="relative mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
                    <a href="{{ route('register') }}" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-zinc-950 px-10 py-4 text-base font-semibold text-zinc-50 shadow-xl transition hover:-translate-y-0.5 hover:bg-brand-50 sm:w-auto">
                        {{ __('landing.cta_primary') }}
                    ...
                    <a href="#pricing" class="inline-flex w-full items-center justify-center rounded-2xl border border-white/20 bg-zinc-950/5 px-8 py-4 text-base font-medium text-zinc-200 backdrop-blur transition hover:border-white/40 hover:text-white sm:w-auto">{{ __('landing.cta_secondary') }}</a>
                </div>
    ...
    {{-- ===== 页脚 ===== --}}
    <footer class="bg-zinc-950 text-zinc-500">
        ...
        <div class="mb-14 h-px rounded-full bg-gradient-to-r from-transparent via-zinc-700 to-transparent"></div>
            ...
            <div class="mt-14 border-t border-zinc-800/80 pt-8 text-center">
                ...
                <a href="https://beian.miit.gov.cn/" target="_blank" rel="noopener noreferrer nofollow" class="mt-2 inline-block text-sm text-zinc-300 transition hover:text-zinc-500">{{ $icp }}</a>
                @endif
            </div>
        </div>
    </footer>
