...
<body class="bg-white font-sans text-zinc-900 antialiased">
    ...
    {{-- ===== 免费 SEO 工具目录（对标 chinaz 工具墙：分类卡片 + 工具直达 chips，数据来自 IndexController 的工具中心 catalog） ===== --}}
    @if ($seoToolsVisible && $seoTools->isNotEmpty())
    <section id="seo-tools" class="border-t border-zinc-100 bg-white py-20 md:py-24">
        <div class="mx-auto max-w-7xl px-6">
            <div class="mx-auto max-w-2xl text-center">
                <h2 class="text-3xl font-bold tracking-tight text-zinc-900 md:text-4xl">{{ __('landing.nav_seo_tools') }}</h2>
                <p class="mt-4 text-lg text-zinc-500">{{ __('landing.seo_tools_section_desc') }}</p>
            </div>
            ...
            <div class="mt-14 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                @foreach ($seoCatOrder as $seoCat)
                    ...
                    <div class="group rounded-2xl border border-zinc-200 bg-zinc-50/50 p-6 transition hover:border-brand-200 hover:bg-white hover:shadow-lg hover:shadow-brand-600/5">
                        <div class="flex items-center gap-3">
                            <span class="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-brand-600 ring-1 ring-zinc-200 transition group-hover:bg-brand-600 group-hover:text-white group-hover:ring-brand-600">{!! $seoCatIcons[$seoCat] ?? $seoCatIcons['dev'] !!}</span>
                            <h3 class="font-semibold text-zinc-900">{{ __("seo.tool_cat_{$seoCat}") }}</h3>
                            <span class="ml-auto rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs font-medium tabular-nums text-zinc-500">{{ $seoCatTools->count() }}</span>
                        </div>
                        <div class="mt-4 flex flex-wrap gap-2">
                            @foreach ($seoCatTools as $slug => $meta)
                                <a href="{{ route('seo.tools.show', $slug) }}" class="rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-sm text-zinc-600 transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700">{{ __("seo.tool_name.{$slug}") }}</a>
                            @endforeach
                        </div>
                    </div>
                    @endif
            ...
            <div class="mt-10 text-center">
                <a href="{{ route('seo.tools') }}" class="inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-zinc-700">
