...
@section('title', __('admin.pages_categories'))
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-zinc-900">{{ __('admin.pages_categories') }}</h1>
    <p class="mt-1 text-sm text-zinc-500">{{ __('admin.pages_categories_hint') }}</p>
</div>
...
<div class="mb-6 rounded-2xl border border-zinc-200 bg-white p-6">
    ...
    <form method="POST" action="{{ route('admin.pages-categories.store') }}" class="flex flex-wrap items-end gap-3">
        @csrf
...
<div class="rounded-2xl border border-zinc-200 bg-white"><div class="overflow-x-auto">
    ...
    <tbody class="divide-y divide-zinc-100">
        ...
        <tr>
            <td class="px-6 py-3 text-zinc-500">{{ $c->page_category_id }}</td>
            ...
            <td class="px-6 py-3 text-right">
                <form method="POST" action="{{ route('admin.pages-categories.destroy', $c->page_category_id) }}" class="inline">@csrf @method('DELETE')<button class="text-sm text-red-500 hover:text-red-700" onclick="return confirm(this.dataset.msg)" data-msg="{{ __('common.confirm_delete') }}">{{ __('common.delete') }}</button></form>
            </td>
        </tr>
