...
<div class="rounded-2xl border border-zinc-200 bg-white p-6">
    ...
    <div class="grid gap-4 md:grid-cols-2">
        ...
        <div><label class="block text-sm font-medium text-zinc-700">月价（{{ \App\Support\Currency::default() }}）</label><input type="number" step="0.01" name="price" value="{{ old('price', $plan->prices['monthly'] ?? 0) }}" class="form-input" required></div>
        <div><label class="block text-sm font-medium text-zinc-700">排序</label><input type="number" name="order" value="{{ old('order', $plan->order ?? 0) }}" class="form-input"></div>
        ...
        <div class="md:col-span-2"><label class="block text-sm font-medium text-zinc-700">描述</label><textarea name="description" rows="2" class="form-input">{{ old('description', $plan->description) }}</textarea></div>
    </div>
