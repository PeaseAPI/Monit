...
<div class="max-w-xl rounded-2xl border border-zinc-200 bg-white p-6">
    ...
    <div class="space-y-4">
        ...
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.domain_scheme') }}</label>
            <select name="scheme" class="form-input"><option value="https" {{ old('scheme', $domain->scheme)==='https' ? 'selected' : '' }}>https</option><option value="http" {{ old('scheme', $domain->scheme)==='http' ? 'selected' : '' }}>http</option></select>
        </div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.domain_host') }}</label><input type="text" name="host" value="{{ old('host', $domain->host) }}" class="form-input" required></div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.domain_type') }}</label>
            <select name="type" class="form-input"><option value="0" {{ old('type', $domain->type)==0 ? 'selected' : '' }}>{{ __('admin.domain_type_user') }}</option><option value="1" {{ old('type', $domain->type)==1 ? 'selected' : '' }}>{{ __('admin.domain_type_platform') }}</option></select>
        </div>
