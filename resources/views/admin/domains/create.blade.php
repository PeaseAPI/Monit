...
<div class="max-w-xl rounded-2xl border border-zinc-200 bg-white p-6">
    ...
    <div class="space-y-4">
        ...
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.domain_scheme') }}</label>
            <select name="scheme" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm"><option value="https">https</option><option value="http">http</option></select>
        </div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.domain_host') }}</label><input type="text" name="host" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm" required placeholder="custom.example.com"></div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.domain_type') }}</label>
            <select name="type" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm"><option value="0">{{ __('admin.domain_type_user') }}</option><option value="1">{{ __('admin.domain_type_platform') }}</option></select>
        </div>
