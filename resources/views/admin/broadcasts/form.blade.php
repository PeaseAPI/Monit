...
<form method="POST" action="{{ $broadcast->exists ? route('admin.broadcasts.update', $broadcast->broadcast_id) : route('admin.broadcasts.store') }}" class="space-y-4 rounded-2xl border border-zinc-200 bg-white p-6">
    ...
    <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.title') }}</label>
        <input type="text" name="title" value="{{ old('title', $broadcast->title) }}" required class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm"></div>
    ...
    <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_target') }}</label>
        <select name="target" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm">
    ...
    <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_plan') }}</label>
        <select name="target_plan_id" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm">
            <option value="">--</option>
    ...
    <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_scheduled_at') }}</label>
        <input type="datetime-local" name="scheduled_at" value="{{ old('scheduled_at', optional($broadcast->scheduled_at)->format('Y-m-d\\TH:i')) }}" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm"></div>
    <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.content') }}</label>
        <textarea name="content" rows="8" required class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm">{{ old('content', $broadcast->content) }}</textarea></div>
