...
<div class="max-w-2xl rounded-2xl border border-zinc-200 bg-white p-6">
    ...
    <div class="space-y-4">
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_name') }}</label><input type="text" name="name" class="form-input" required></div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_subject') }}</label><input type="text" name="subject" class="form-input" required></div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_content') }}</label><textarea name="content" rows="10" class="form-input" required></textarea></div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_type') }}</label>
            <select name="type" class="form-input"><option value="email">{{ __('admin.broadcast_type_email') }}</option><option value="push">{{ __('admin.broadcast_type_push') }}</option></select>
        </div>
