...
<div class="max-w-2xl rounded-2xl border border-zinc-200 bg-white p-6">
    ...
    <div class="space-y-4">
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_name') }}</label><input type="text" name="name" value="{{ old('name', $broadcast->name) }}" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm" required></div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_subject') }}</label><input type="text" name="subject" value="{{ old('subject', $broadcast->subject) }}" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm" required></div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.broadcast_content') }}</label><textarea name="content" rows="10" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm">{{ old('content', $broadcast->content) }}</textarea></div>
