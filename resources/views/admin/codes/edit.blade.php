...
<div class="max-w-xl rounded-2xl border border-zinc-200 bg-white p-6">
    ...
    <div class="space-y-4">
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.code_name') }}</label><input type="text" name="name" value="{{ old('name', $code->name) }}" class="form-input" required></div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.code_code') }}</label><input type="text" name="code" value="{{ old('code', $code->code) }}" class="form-input font-mono" required></div>
        ...
        <div id="discount-field" class="{{ old('type', $code->type) === 'plan' ? 'hidden' : '' }}"><label class="block text-sm font-medium text-zinc-700">{{ __('admin.code_discount') }} (%)</label><input type="number" name="discount" min="0" max="100" value="{{ old('discount', $code->discount) }}" class="form-input"></div>
        <div id="plan-field" class="{{ old('type', $code->type) !== 'plan' ? 'hidden' : '' }}">
            <div class="grid gap-4 md:grid-cols-2">
                <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.code_plan') }}</label><input type="text" name="plan_id" value="{{ old('plan_id', $code->plan_id) }}" class="form-input"></div>
                <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.code_days') }}</label><input type="number" name="days" min="0" value="{{ old('days', $code->days) }}" class="form-input"></div>
            </div>
        </div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.code_max_redemptions') }}</label><input type="number" name="max_redemptions" min="0" value="{{ old('max_redemptions', $code->max_redemptions) }}" class="form-input"></div>
