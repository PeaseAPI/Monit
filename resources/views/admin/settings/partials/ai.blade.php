...
<div class="space-y-4">
    ...
    <div>
        ...
        <select name="ai_provider" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm">
    ...
    <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.ai_api_key') }}</label>
        <input type="password" name="ai_api_key" value="{{ old('ai_api_key', $settings['ai.ai_api_key'] ?? '') }}" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm" autocomplete="new-password">
    ...
    <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.ai_model') }}</label>
        <input type="text" name="ai_model" value="{{ old('ai_model', $settings['ai.ai_model'] ?? '') }}" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm" placeholder="{{ $providers[old('ai_provider', $settings['ai.ai_provider'] ?? 'log')]['default_model'] ?? '' }}">
    ...
    <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.ai_base_url') }}</label>
        <input type="text" name="ai_base_url" value="{{ old('ai_base_url', $settings['ai.ai_base_url'] ?? '') }}" class="mt-1 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm" placeholder="https://api.deepseek.com/v1">
    ...
    <div class="grid grid-cols-3 gap-4">
        <div><label class="block text-sm text-zinc-600">{{ __('admin.ai_temperature') }}</label>
            <input type="number" step="0.1" min="0" max="2" name="ai_temperature" value="{{ old('ai_temperature', $settings['ai.ai_temperature'] ?? 0.7) }}" class="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm"></div>
        <div><label class="block text-sm text-zinc-600">{{ __('admin.ai_max_tokens') }}</label>
            <input type="number" min="16" max="32768" name="ai_max_tokens" value="{{ old('ai_max_tokens', $settings['ai.ai_max_tokens'] ?? 1024) }}" class="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm"></div>
        <div><label class="block text-sm text-zinc-600">{{ __('admin.ai_timeout') }}</label>
            <input type="number" min="5" max="300" name="ai_timeout" value="{{ old('ai_timeout', $settings['ai.ai_timeout'] ?? 60) }}" class="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm"></div>
    </div>
