...
<div class="max-w-xl rounded-2xl border border-zinc-200 bg-white p-6">
    ...
    <div class="space-y-4">
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.tax_country') }}</label><input type="text" name="country" value="{{ old('country', $tax->country) }}" class="form-input" required></div>
        <div><label class="block text-sm font-medium text-zinc-700">{{ __('admin.tax_rate') }}</label><input type="number" step="0.01" name="rate" value="{{ old('rate', $tax->rate) }}" class="form-input" required></div>
