        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-zinc-100">
                ...
                <tr class="hover:bg-zinc-50">
                    ...
                    <td class="px-4 py-3">{{ $payment->payment_processor }}</td>
                    <td class="px-4 py-3">
                        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium
                            {{ $payment->status === 1 ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700' }}">
                            {{ $payment->status === 1 ? __('payments.status_paid') : __('payments.status_pending') }}
                        </span>
                    </td>
